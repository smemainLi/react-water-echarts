...
import { earlyWarmImg, unitImg, waterLevelImg, contactImg, phoneImg } from '../HighPosition';
import { immersionWarm } from '../../utils/request';
...
const WaterLogging: FC = () => {
  ...
  const getData = useCallback<any>(async () => {
    try {
      const result = await immersionWarm({ pageNo: 1, pageSize: 20 })
      setData(result.data);
    } catch (error) { }
  }, [])

  useEffect(() => {
    document.title = "水浸预警信息";
    getData();
  }, []);
  ...
  return (
    <div className='water-logging-main-body'>
